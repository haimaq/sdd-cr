     ...
     initializeList: function (fieldName,dropDownName, noneString) {
	     ...
	     var dropdownList = jQuerySBM('#'+dropdownId);
             jQuerySBM(dropdownList).find('option').remove();
              var reportRef = "TSM_CHANGEREQUEST.CPM.ReadyToLink";
			...
			else {jQuerySBM(dropdownList).prepend( "<option value=0>None</option>" ).select();
			...
			jQuerySBM(results).each(function( intNameIndex ){ 
				...
				SOO.changeProposalSelector.updateField(fieldName, 0, noneString);					     				 
				}
			...
			jQuerySBM(dropdownList).change(function() {
				  ...
				  var dpTableId = jQuerySBM('#'+dropdownId+' option:selected').attr('tableid');
                			  SOO.changeProposalSelector.updateField(fieldName, dropdownValue, dropdownText);
			 	if (dropdownValue != 0) {
                                           ShowField("loadingDPText");
				  	}
				else{MakeFieldInvalid("CHANGE_PACKAGE");}
