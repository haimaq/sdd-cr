...
SOO.Util = {
  ...
  isConfigurationManagementSystemDeployed: function() {
    return SOO.Util.isAppDeployed(
      SOO.Util.Applications.ConfigurationManagementSystem
    );
  },
  ...
  openAddToFolderPopup: function(tableID, recordID) {
    ...
    try {
      top.Serena.View.allViews.renderAddToFoldersMenu(
        view,
        top.Serena.Favorites.AddToFolders.Params.any("item")
      );
    } catch (e) {
      sbmlog.error(
        "Exception occurred while trying to add item [tableID: " +
          tableID +
          ", recordID: " +
          recordID +
          "] to folder. Error: " +
          e.messsage,
        "openAddToFolderPopup"
      );
    }
  },
  ...
  getItemData: function(
    ...
    var request =
      "tmtrack.dll?JSONPage&Command=viewitem&projectid=" +
      projectId +
      "&recordid=" +
      recordId +
      "&tableid=" +
  ...
  getReportJson: function(reportRef, reportParams, rowsOnly, callback) {
    ...
    var request =
      "tmtrack.dll?ReportPage&template=reports%2Fjsonlist&ReportRef=" +
      reportRef +
      "&HasRuntimeParams=" +
      hasParams +
      paramStr;
  ...
  createViewItemLink: function(itemId, tableId, linkStr, target) {
    target =
      target == null || target == undefined || target == "" ? "_blank" : target;

    return (
      "<a href='tmtrack.dll?IssuePage&RecordId=" +
      itemId +
      "&Template=view&TableId=" +
      tableId +
      "' target='" +
      target +
      "'>" +
...
SOO.Plugins = {
  ...
  ImagePicker: {
    load: function() {
      SOO.Util.loadCss(
        "styles/shell/srp/theme/jquery-ui-1.8.24.custom.min.css"
      );
      SOO.Util.loadJavaScript(
        "javascript/shell/common/jquery-ui-1.8.24.min.js"
      );
...
SOO.ShellFormatting = {
  ...
  isInSrp: function() {
    return this.isInShell("srp");
  },
  isInSdc: function() {
    return this.isInShell("sdc");
  },
  isInSwc: function() {
    return this.isInShell("swc");
  },
  ...
  formatSOOForm: function() {
    ...
    jQuerySBM("#contentTablex td:first, #SectionWrapper").css(
      "font-size",
      "0px"
    );
    ...
    jQuerySBM("[id$='GreyBorderStyle']").css(
      "border",
      "1px solid " + this.greyBorderColor
    );

    //Style groupbox
    jQuerySBM(
      "table[id$='GroupBox'] img[src$='tab_transparent_content_NE.gif']"
    ).attr(
      "src",
      SOO.ShellFormatting.rootImagePath + "tab_transparent_content_NE.gif"
    );
    jQuerySBM(
      "table[id$='GroupBox'] img[src$='tab_transparent_content_NW.gif']"
    ).attr(
      "src",
      SOO.ShellFormatting.rootImagePath + "tab_transparent_content_NW.gif"
    );
    jQuerySBM(
      "table[id$='GroupBox'] img[src$='tab_transparent_content_SE.gif']"
    ).attr(
      "src",
      SOO.ShellFormatting.rootImagePath + "tab_transparent_content_SE.gif"
    );
    jQuerySBM(
      "table[id$='GroupBox'] img[src$='tab_transparent_content_SW.gif']"
    ).attr(
      "src",
      SOO.ShellFormatting.rootImagePath + "tab_transparent_content_SW.gif"
    );

    jQuerySBM(".multiselectborder td[nowrap]").attr("colspan", 3);
    jQuerySBM(".multiselectborder").attr("style", "width: 262px");
    jQuerySBM(".multiselectborder").css(
      "background-color",
    ...
    if (ft === this.formType_Submit) {
      this.formatSOOSubmitForm();
    } else if (ft === this.formType_Transition) {
      this.formatSOOTransitionForm();
    } else if (ft === this.formType_State) {
      this.formatSOOStateForm();
    } else {
      // Invalid form type
    ...
    setTimeout(function() {
      if (ft === SOO.ShellFormatting.formType_State) {
        ...
        if (!SOO.ShellFormatting.isQuickView()) {
          jQuerySBM("#" + SOO.ShellFormatting.defaultSidebarSection).css(
            "height",
            sfh - parseInt(jQuerySBM("#StatePanel").height())
          );
          ...
          shadowPanel
            ...
            .css(
              "border-left",
              "solid 1px " + SOO.ShellFormatting.greyBorderColor
            );
  ...
  formatSOOStateForm: function() {
    ...
    jQuerySBM("#myzoom").attr(
      "href",
      "javascript:parent.zoomReport(" + name + ");"
    );
  ...
  formatStatePanel: function() {
    ...
    jQuerySBM("#" + SOO.ShellFormatting.Sidebar.expandedPanelName).width(
      jQuerySBM("#StatePanel").innerWidth() - 2
    );
    jQuerySBM("#StatePanel").width(
      jQuerySBM("#" + SOO.ShellFormatting.Sidebar.expandedPanelName).width() + 2
    );
  },
  ...
  formatSOOTransitionForm: function(isSubmit) {
    ...
    if (topFrameset.length > 0) {
      if (isSubmit) {
        topFrameset[0].rows = "0,*";
      } else {
        topFrameset[0].rows = "0,0,*";
      }
    }
  ...
  Sidebar: {
    ...
    getDefaultState: function() {
      var cookieVal = SOO.Util.readCookie(
        SOO.ShellFormatting.Sidebar.stateCookieName
      );

      //Default tp expanded
      return cookieVal == SOO.ShellFormatting.Sidebar.collapsedStateValue
        ? SOO.ShellFormatting.Sidebar.collapsedStateValue
        : SOO.ShellFormatting.Sidebar.expandedStateValue;
    },

    setToDefaultState: function() {
      if (
        SOO.ShellFormatting.Sidebar.getDefaultState() ==
        SOO.ShellFormatting.Sidebar.collapsedStateValue
      ) {
        SOO.ShellFormatting.Sidebar.collapse(false, false);
      } else {
    ...
    collapse: function(blnAnimate, blnSaveState) {
      ...
      if (blnAnimate) {
        jQuerySBM("#" + SOO.ShellFormatting.Sidebar.expandedPanelName).hide(
          "slide",
          {},
          SOO.ShellFormatting.Sidebar.animationSpeed
        );
        jQuerySBM("#" + SOO.ShellFormatting.Sidebar.collapsedPanelName).show(
          "slide",
          {},
          SOO.ShellFormatting.Sidebar.animationSpeed
        );
      ...
      if (blnSaveState) {
        SOO.Util.createCookie(
          SOO.ShellFormatting.Sidebar.stateCookieName,
          SOO.ShellFormatting.Sidebar.collapsedStateValue,
          SOO.ShellFormatting.Sidebar.stateSaveDays
        );
      }
    },
    ...
    expand: function(blnAnimate, blnSaveState) {
      ...
      if (blnAnimate) {
        ...
        jQuerySBM("#" + SOO.ShellFormatting.Sidebar.expandedPanelName).show(
          "slide",
          {},
          SOO.ShellFormatting.Sidebar.animationSpeed
        );
 ...
 MoreActions: {
    ...
    setupMoreActions: function(name) {
      ...
      $link.mouseenter(function() {
        if (!SOO.ShellFormatting.MoreActions.moreActionsInitialized) {
          SOO.ShellFormatting.MoreActions.initMoreActions(name,
            className);
        }
        ...
        $pop.css({
          "left": ((pos.left + scrollX + width) - $pop.width() + 17) +
            "px",
          "top": (pos.top + scrollY + 31) + "px"
        }).stop(true, true).show();
      });
    ...
    .addClass(className)
      ...
      actionCount = 0;
      jQuerySBM("a#refresh_view img, a#printable img,a#emaillink img,a img#copyurl, a#display_associated_workflow img, a#display_help img")
        .each(function(index) {
          if (index == 0) return; // skip first option
  ...
  Tabs: {
    applyNewTabStyle: function(name) {
      ...
      $tab
        .find(".tabCell4Spacer:last")
        .removeClass("tabCell4Spacer")
        .addClass("tabCell4InActiveLeft");
      $tab
        .find(".tabCell4Spacer:first")
        .removeClass("tabCell4Spacer")
        .addClass("tabCell4InActiveRight")
        .html(" ");
    },
    // start wizard
    ...
    wizardSelectionChanged: function(
      sectionid,
      groupid,
      formInfoObj,
      newTab,
      ...
      if (newTab) {
        var test = jQuerySBM(newTab).nextAll(
          ".tabCell4InActiveLeft,.tabCell4ActiveLeft"
        );
        var $next = null;
      ...
      if (oldTab) {
        var test = jQuerySBM(oldTab).nextAll(
          ".tabCell4InActiveLeft,.tabCell4ActiveLeft"
        );
        var $next = null;
        test.each(function(index, el) {
...
SOO.CustomFormActions = {
  ...
  compareDateTimes: function(date1, date2, operator) {
    ...
    if (date1Value == null || date1Value == "" || date1Value == undefined) {
      return true;
    }
    if (date2Value == null || date2Value == "" || date2Value == undefined) {
      return true;
    }
    if (subType == "date") {
    ...
    if (date1Value > date2Value) {
      return true;
    } else if (date1Value >= date2Value && operator == "equalorgreater") {
      return true;
    } else if (date1Value.valueOf() == 0) {
      return true;
    } else {
      return false;
    }
...
function isOrphanSingleList(fld) {
  ...
  return (
    $fld.is("select") && // not readonly/checkboxes
...
function fillOrphanField(fld) {
  ...
  qbe = qbe.replace(
    /^([^,]+),document\.forms\[0\]\.(F[0-9]+),document\.forms\[0\]\.(G[0-9]+),'([0-9]+)'/,
  ...
  $fld
    .attr("multiselect", true)
    .attr(
      "ondblclick",
      "multiSelectRemove('" + fld.fid + "', '" + fld.fid + "_LEFT' )"
    );
  ...
  jQuerySBM(
    '<td align="left">' +
      ...
      '_LEFT" name="MULTI_' +
      fld.fid +
      '_LEFT" size="5" MULTIPLE ondblclick="multiSelectAdd(\'' +
      fld.fid +
      "', '" +
      fld.fid +
      '_LEFT\')" class="' +
  ...
  jQuerySBM(
    '<td align="center" style="width:1px;">' +
      ...
      "<tr>" +
      '<td valign="bottom">' +
      '<a class="nounderline" href="javascript:multiSelectAdd(\'' +
      fld.fid +
      "', '" +
      fld.fid +
      '_LEFT\')" title="Add to selected items">' +
      '<img alt="Add to selected items" src="images/' +
      sTheme +
      '/form_arrow_right.gif" align="absmiddle" border="0" />' +
      "</a>" +
      "</td>" +
      "</tr>" +
      "<tr>" +
      '<td valign="top">' +
      '<a class="nounderline" href="javascript:multiSelectRemove(\'' +
      fld.fid +
      "', '" +
      fld.fid +
      '_LEFT\')" title="Remove from selected items">' +
      '<img alt="Remove from selected items" src="images/' +
      sTheme +
      '/form_arrow_left.gif" align="absmiddle" border="0" />' +
      "</a>" +
      "</td>" +
      "</tr>" +
      "</table>" +
...
if (isMobileWebKit()) {
  for (var i = 0; i < aFieldLookup.Fields.length; i++) {
    ...
    if (isOrphanSingleList(fld)) fillOrphanField(fld);
  }
}
// fix for wordbreak on responsive forms
function SOOFixResponsivePrefSuf() {
  if (isChrome()) {
    // Chrome gets by with word-break: break-all.
    return;
  }
  jQuerySBM(".expanderRoot, .genericTabContent, .hideCell").addClass(
    "EmbeddedReportResizeShow"
  );
  jQuerySBM(".SOOPrefSufAdjust .rofld").css("width", "auto");
  jQuerySBM(".SOOPrefSufAdjust").removeClass("SOOPrefSufAdjust");

  jQuerySBM(".rofld").each(function() {
    //Widget > .PrefSufTable').each(function () {
    var $rofld = jQuerySBM(this);
    var $widget = $rofld.closest(".fieldWidget");
    var $table = $widget.find("table");
    if (!$table.length || !$rofld.parent().is("td")) {
      // only need to operate on prefix/suffix tables
      return;
    }
    ...
    if ($cell.children(".doubleCell").length > 0) {
      // may or may not be in doublecell
    ...
    if ($table.outerWidth() <= $cell.outerWidth()) {
      // no change needed
      return;
    }
  ...
  jQuerySBM(".EmbeddedReportResizeShow").removeClass(
    "EmbeddedReportResizeShow"
  );
}
...
function SOOFixSearchableFieldsCF(forceReload) {
  ...
  for (var i = 0; i < aFieldLookup.Fields.length; i++) {
    ...
    if (
      ...
      if (LookupFieldSubType(fld.fid) == "duration") {
        // convert to ms
        val *= 1000;
      } else if (LookupFieldType(fld.fid) == "date") {
        // convert to primitive (IE memory bug)
        val = Number(val);
      } else if (
        $fld.is("select") &&
        (fld.type == "single" || fld.type == "multi")
      ) {
        // searchable -- preserve full values
        val = { options: [] };
        if (fld.type == "multi") {
        ...
        } else if (!IsFieldEmpty(fld.fid)) {
          // single
          val.options.push({
            name: GetFieldValue(fld.fid),
            id: parseInt(GetFieldValue(fld.fid, 0, "ID"))
          });
        }
      }
...
function SOOFixSearchableFieldsLCF() {
  ...
  if (
    ...
    var formstoreoption = top
      .getUserPreference(top.USERSETTING_SYS_FORMRESTOREOPTION, "Always")
      .toLowerCase();
    if (
      r.modernFormCache.forceReload ||
      formstoreoption == "always" ||
      (formstoreoption == "prompt" &&
        confirm(ids_msg_itemview_confirmpopulateform))
    ) {
      for (var key in r.modernFormCache.values) {
        var val = r.modernFormCache.values[key];
        // re-convert dates (IE memory bug)
        if (
          LookupFieldType(key) == "date" &&
          LookupFieldSubType(key) != "duration"
        ) {
          val = new Date(val);
        } else if (val.options) {
          // repopulate blank dropdowns
          ...
          for (var i = 0; i < val.options.length; i++) {
            ...
            if (
              $leftBox.length &&
              !$leftBox.find('option[value="' + val.options[i].id + '"]').length
            ) {
              $leftBox[0].options[$leftBox[0].options.length] = new Option(
                val.options[i].name,
                val.options[i].id
              );
            }
          ...
          for (var i = fld.options.length - 1; i >= 0; i--) {
            if (
              !(fld.options[i].innerText || fld.options[i].textContent).trim()
            ) {
              fld.remove(i);
            }
          }
